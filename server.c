...
void *client_output(void *arg)
{
    ...
    while (1)
    {
        ...
        if (last_message != NULL)
        {
            ...
            else if (strcmp(output, last_message) != 0)
            {
                strcpy(output, last_message);
    ...
    while (1)
    {
        ...
        if (current_user == NULL)
        {
            ...
            if (token != NULL)
            {
                ...
                token = strtok(NULL, " ");
                if (token != NULL)
                    strncpy(arg_b, token, DEFAULT_BUFLEN);
                token = strtok(NULL, " ");
                if (token != NULL)
                    strncpy(arg_c, token, DEFAULT_BUFLEN);
            }
            arg_a[strcspn(arg_a, "\n")] = 0;
            arg_b[strcspn(arg_b, "\n")] = 0;
            arg_c[strcspn(arg_c, "\n")] = 0;
            ...
            else if (strcmp(arg_b, "") && strcmp(arg_c, ""))
            {
                ...
                if (current_user != NULL)
                {
                    ...
                    write(client_socket, output, strlen(output));
                    if (pthread_create(&output_thread, NULL, client_output, &client_socket))
                    {
                        write(client_socket, "Error creating output thread.\nWarning: You will not receive messages.\n", 71);
                    }
                    else
                    {
